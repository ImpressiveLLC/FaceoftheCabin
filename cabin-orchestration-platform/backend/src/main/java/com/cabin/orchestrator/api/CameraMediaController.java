...
public class CameraMediaController {
    ...
    // 2026-08-03, extracted into BlinkLiveviewService 2026-08-16 so
    ...
    private ResponseEntity<?> proxyLiveviewControl(String cameraName, String action) {
        BlinkLiveviewService.Result result = "start".equals(action)
            ? liveviewService.start(cameraName)
            : liveviewService.stop(cameraName);
        if (result.skipped()) {
            return ResponseEntity.ok(Map.of("ok", true, "skipped", true));
        }
        if (!result.ok()) {
            return ResponseEntity.status(HttpStatus.BAD_GATEWAY)
                .body(result.error() != null ? Map.of("ok", false, "error", result.error()) : Map.of("ok", false));
        }
        return ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON).body(result.body());
    }
