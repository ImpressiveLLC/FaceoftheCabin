...
| **Route format**   | **Amended 2026-09-23 ([DL-2026-09-23-02](../discrepancy-log.md), ratified deviation — W-2 passed, 49/49 green in [PR #100](https://github.com/ImpressiveLLC/FaceoftheCabin/pull/100)):** `/view/{token}` is a client-side route only — `GuestDashboard` (React), not a separate backend controller. It calls the *same* shared `/api/*` endpoints every signed-in caller uses, appending `?t={token}`. The barrier is entirely server-side, in `GoogleAuthInterceptor.handleGuestToken()`: GET-only (any write 403s before scope is even checked) plus a fixed scope-to-path-prefix map (`SCOPE_PATH_PREFIXES`) — a route not covered by any granted scope 403s regardless of method. No `X-Robots-Tag` is set; nothing under `/view/` is server-rendered to set a response header on in the first place.                                                                                                                                                                                                                                                                                                                                   |
...
4. All real reporting shows. Updated by Nate 2026-09-24: non-presence history aligns with the normal view, up to 60 days (Q-DM-2 answered).
...
| R-DM-8 | **History and reporting.** `telemetry-history` and `reported-fields` are `ALLOW_REDACT`: normal series for non-presence devices, empty series for presence-class devices, never a client error. `days` is clamped to `cabin.demo.max-history-days`, default **60**, aligned with the normal view maximum (Nate, 2026-09-24; Q-DM-2 answered). The clamp remains a ceiling; presence-class series stay empty. |
...
| Q-DM-2 | **Answered — Nate, 2026-09-24:** demo history aligns with the normal view. Set `cabin.demo.max-history-days` to **60**; retain the ceiling and empty presence-class series per R-DM-8. | Nate |
