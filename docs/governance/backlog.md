...
| W-5 | Frigate `front_door` before/after measurement: record 24 h baseline (Reolink motion count vs Frigate events vs continuous recording) before merging branch `claude-code/frigate-front-door-motion-sensitivity` (`7d014d2`), then repeat 24 h after | UC-7, UC-2 | D18 | ad-hoc | Code | 10+5+4+6 = 25 | 2 | **12.5** | **gated** | W-15 (no live `front_door` feed to measure) |
...
| W-15 | Reconnect `front_door` (Reolink) to Frigate across the Starlink network split. Scope A first, as a reviewed plan ([execution plan](../EXECUTION_PLAN_2026-09-24_W-15-front-door-reconnect.md)): Step 0 probes the camera's current address from the M920q (no network change) — **run 2026-09-24: camera is at `192.168.1.121` and is not reachable from the M920q's `192.168.2.0/24`, so Step 1 is pending Nate's approval**; Step 1 is a time-boxed `never-default` join of the M920q's single Wi-Fi radio with timed auto-revert. Camera is online on Starlink's 2.4 GHz side while the M920q is on 5 GHz (Nate, 2026-09-24). **Blocks W-5 and [#97](https://github.com/ImpressiveLLC/FaceoftheCabin/pull/97), which is held.** No host network change without Nate present | UC-7, UC-2 | D18 | ad-hoc | Code plans; **Nate** present for any host network change | 14+4+3+9 = 30 — proposed by Code, Cowork to confirm (S7: a camera coverage gap, not a life-safety sensor; T9: active claim and renewal window). Sized 3, re-size after Step 0 | 3 | **10.0** | proposed | — |
