...
| G11 | Ontology D17 (`device_room`↔`hub_location` edge, `data_class: product` lineage tier, new `knowledge_node`/`knowledge_chunk_type` entities) is implemented in `docs/ontology.yaml` (commit `c925a03`, 2026-09-11) but not yet ratified in the shared artifact; D18 (camera-derived media naming), D19 (local telemetry backup/log-shipping), and D20 (Live MQTT relay) were proposed 2026-09-11 through 2026-09-13 and remain unreviewed there too. Found 2026-09-13 while reconciling this session's Home-collector work against the artifact — see `docs/ai-assistant/wsjf-backlog.md`'s DEP09. | Re-check the artifact's Discrepancy Log for current status before citing any of D17–D20 as settled in a K16 answer. | K16 (Ontology/provenance) answer accuracy only |
