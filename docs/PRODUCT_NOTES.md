...
## 2026-08-14 — Current Alerts Must Be Current, In-Scope, and Server-Owned

_Prompted by the live product showing warning state that did not align with
the device lifecycle ontology, plus a Rules & Alerts sidebar that still made
configuration and delivery claims the app could not prove._

**Event history is not active state.** A recent CRITICAL `CabinEvent` remains
valuable evidence after a leak clears, but it must not keep a current-alert
badge lit. `GET /api/alerts/active` therefore projects only present runtime
evidence. It does not read the latest event and guess that the condition is
still true.

**Visibility does not grant monitoring authority.** Candidates stay visible
for review and `AVAILABLE` devices are in scope but unassigned. Neither state
can produce an active alert. A device participates only when its descriptor is
enabled and its person-controlled lifecycle is `ASSIGNED`. `DEFERRED`,
`IGNORED`, disabled, candidate, and available-but-unassigned devices are
excluded even if an old runtime state still says `OFFLINE`.

**The two current conditions have deliberately narrow semantics.** A current
`DeviceStatus.state == ALARM` is `CRITICAL`. An authoritative
`CheckinStatus == MISSED` is `WARN`. Merely `LATE` is not an alert, raw
`OFFLINE` is not independently promoted, and the browser no longer converts a
warning to critical after 20 minutes. If a device is both alarming and missed,
the alarm wins so a single device is not double-counted.

**The browser is a projection, not a second alert engine.** The old
localStorage opt-in, Reset Alerts control, client timer, and aggregate
`/api/system/health` inference were removed. Navigation badges, the panel
status bar, and the new current-conditions list all consume the same active
alert response. An unreachable endpoint displays “status unavailable” and
does not fabricate an all-clear or a warning.

**Rule visibility now names ownership and configuration truthfully.**
`GET /api/alerts/rules` is generated by `AutomationRuleService` from its real
thresholds and executable rule branches. The UI labels those entries as
`CABIN_BACKEND`, read-only, and either deploy-time or code configured. It does
not imply that a Node-RED or Home Assistant flow exists, and it says CRITICAL
notification delivery runs only when a channel is configured. Editing safety
